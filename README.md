...
The professor node drives a virtual vehicle around a figure-8-shaped trajectory (two lobes of radius ~8 m) and publishes noisy IMU + noisy GPS + noise-free ground truth. Your job is to publish an odometry estimate that the professor's grader can compare against truth. The grader auto-discovers your topics on the shared class network and posts pass/fail verdicts on `/professor/feedback`.
...
3. Verify you can reach the professor's node: `tailscale ping professor`.
...
export A4_PROFESSOR_HOST=<professor tailnet host>    # e.g. professor.tail1234.ts.net
...
`dt` should come from the difference between consecutive IMU header timestamps — do **not** use wall-clock time, since the professor stamps everything with simulated time (this is what makes grading reproducible).

### 2.2 Where to put your code
Open `ros2_ws/src/a4_student/a4_student/dead_reckoning_node.py`. There's a `TODO(student)` block inside `_on_imu`. Replace the stub with the integration above.

### 2.3 Run it
```bash
colcon build --symlink-install
source install/setup.bash
ros2 launch a4_student dr.launch.py github_user:=$GITHUB_USER
```
...
**Deliverable for A4.1:** screenshot of `/professor/feedback` congratulating your handle, committed as `submissions/a4_1_feedback.png`, plus your finished `dead_reckoning_node.py`.
...
A reasonable starting point for the EKF tuning constants:
...
Open `ros2_ws/src/a4_student/a4_student/ekf_node.py`. There is a plain-numpy `EKF` class with `predict()` and `update_gps()`. The ROS node calls them for you; you just have to fill in the math.

### 3.4 Run it
```bash
colcon build --symlink-install
source install/setup.bash
ros2 launch a4_student ekf.launch.py github_user:=$GITHUB_USER
```

### 3.5 How A4.2 grading works
The grader:
1. Buffers `/professor/truth`.
...
4. Computes 2D position RMSE. If `RMSE < 0.5 m` it publishes on `/professor/feedback`:
   ```
...
**Deliverable for A4.2:** screenshot of `/professor/feedback` congratulating your handle, committed as `submissions/a4_2_feedback.png`, plus your finished `ekf_node.py`.
...
3. Add a **3D** panel and enable these topics:
   - `/professor/truth` (green — ground truth path).
   - `/professor/gps` (red dots — noisy GPS).
...
| ------------------------ | --------------------------- | -------- | ----------------------------------------------- |
| `/professor/imu`         | `sensor_msgs/Imu`           | Professor | Noisy IMU (50 Hz)                              |
| `/professor/gps`         | `geometry_msgs/PoseStamped` | Professor | Noisy GPS (5 Hz)                               |
| `/professor/truth`       | `nav_msgs/Odometry`         | Professor | Noise-free ground truth (50 Hz)                 |
...
├── ros2_ws/
│   └── src/
│       ├── a4_student/        # your template — this is where you write code
│       └── a4_professor/      # for reference; not run by students
└── README.md
```

## 7. Troubleshooting

- **`ros2 topic list` doesn't show `/professor/imu`.** DDS discovery isn't reaching the professor. Confirm `tailscale ping <professor-host>` works, `A4_PROFESSOR_HOST` is set, and `ROS_DOMAIN_ID` matches (`42`).
- **You see your own topics but no one else's.** Check `RMW_IMPLEMENTATION=rmw_cyclonedds_cpp` inside the container (`env | grep RMW`).
- **DR fails even though the code looks right.** Two common bugs: (1) computing `dt` from `time.time()` instead of `msg.header.stamp` — the professor uses simulated time; (2) updating position with `v_old * cos(theta_new)` when semi-implicit Euler wants `v_new * cos(theta_new)`.
